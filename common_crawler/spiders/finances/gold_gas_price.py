...
GOLD_PRICE_RECORD_ID = "prices.gold"
GAS_PRICE_RECORD_ID = "prices.gasoline"

GOLD_DISCORD_LAST_MSG_ID = "discord.gold.last_msg_id"
GAS_DISCORD_LAST_MSG_ID = "discord.gasoline.last_msg_id"
...
class FinancePipeline:
    ...
    async def process_item(self, item, spider):
        ...
        curr_change_abs = item.get("change_abs", 0)
        limit = (
            GOLD_CHANGE_THRESHOLD
            if item["id"] == GOLD_PRICE_RECORD_ID
            else GAS_CHANGE_THRESHOLD
        )

        prev_price = prev_item.get("price", 0) if prev_item else 0
        prev_change_abs = prev_item.get("change_abs", 0) if prev_item else 0

        is_breaking_price = False
        if abs(curr_change_abs) > limit:
            if abs(prev_change_abs) <= limit:
                # Just crossed the threshold
                is_breaking_price = True
            elif abs(curr_price - prev_price) >= limit:
                # Already above, but moved significantly further (another threshold unit)
                is_breaking_price = True

        if is_breaking_price:
            reasons.append(
                f"🚨 Significant price change ({curr_change_abs}) exceeds threshold ({limit})"
            )
    ...
    async def send_price_notification(self, item, reasons):
        webhook_url = self.gold_webhook_url
        if not webhook_url:
            return

        discord = DiscordNotifier(webhook_url)

        # Handle old message deletion
        msg_record_id = (
            GOLD_DISCORD_LAST_MSG_ID
            if item["id"] == GOLD_PRICE_RECORD_ID
            else GAS_DISCORD_LAST_MSG_ID
        )
        last_msg_doc = self.db[COLLECTION_NAME].find_one({"id": msg_record_id})
        if last_msg_doc:
        ...
        if msg_id:
            self.db[COLLECTION_NAME].update_one(
                {"id": msg_record_id}, {"$set": {"last_msg_id": msg_id}}, upsert=True
            )
